(function(){
  const apiUrl = apiProductsUrl; // apiProductsUrl از قالب HTML میاد
  const defaultImage = defaultProductImage; // defaultProductImage از قالب HTML میاد
        ...
        <a href="/product/${p.id}/${p.slug}/" class="block">
          <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 product-card h-full flex flex-col" data-id="${p.id}">
            ...
            <div class="p-4 flex-1 flex flex-col">
              <h3 class="text-lg font-bold text-gray-800 mb-2 line-clamp-2 leading-relaxed">${escapeHtml(p.name)}</h3>
              <p class="text-gray-600 text-sm mb-3 flex-1 line-clamp-3 leading-relaxed">${description}</p>
              <div class="mt-auto">
  ...
  window.addEventListener('popstate', ()=>{ const page = parseInt(new URLSearchParams(window.location.search).get('page') || '1'); loadProducts(page,false); });
